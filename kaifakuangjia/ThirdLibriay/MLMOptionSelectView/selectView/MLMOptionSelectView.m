...
- (instancetype)initOptionView {
    if (self = [super initWithFrame:CGRectZero style:UITableViewStylePlain]) {
        ...
        self.bounces = NO;
        self.separatorColor = kColor_SeparateLineColor;
...
- (void)initSetting {
    ...
    _coverColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.3];
...
-(void)viewDidLayoutSubviews
{
    if ([self respondsToSelector:@selector(setSeparatorInset:)]) {
        [self setSeparatorInset:UIEdgeInsetsMake(0,4,0,4)];
    }
    
    if ([self respondsToSelector:@selector(setLayoutMargins:)]) {
        [self setLayoutMargins:UIEdgeInsetsMake(0,4,0,4)];
    }
}
